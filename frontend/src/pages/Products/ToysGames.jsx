...
function ToysGames() {
  ...
  useEffect(() => {
    ...
    const fetchToys = async () => {
      try {
        const res = await fetch("https://shopeasy-backend-sagk.onrender.com/api/products?category=ToysGames");
        if (!res.ok) throw new Error("Failed to fetch products");
        const data = await res.json();
        const productsArray = data.products || data || [];
        setToysProducts(productsArray);
  ...
  return (
    <div className="min-h-screen text-black bg-gray-100 p-6">
      ...
      <ProductFilter
        ...
        brands={[...new Set(toysProducts.map(p => p.brand).filter(Boolean))]}
        priceRange={{ min: 0, max: Math.max(...toysProducts.map(p => Number(p.price) || 0)) }}
      />
          ...
          >
            ...
            >
              <div className="w-full h-48 bg-gray-50 rounded mb-4 overflow-hidden flex items-center justify-center">
                <img
                  src={(product.images && product.images[0]) || product.imageUrl || ToysBanner}
                  alt={product.name}
