...
import BookImage from "../../assets/Images/BookImage.avif";
    ...
    <div className="min-h-screen text-black bg-gray-100 p-6">
      ...
      >
        ...
        <div className="relative z-10 px-4">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-3 sm:mb-4 drop-shadow-lg">Books</h1>
          <p className="text-base sm:text-lg mb-4 sm:mb-6 drop-shadow-md">Explore our range of books.</p>
          <Link
            to="/"
      ...
      <ProductFilter
        ...
        brands={[...new Set(books.map(p => p.brand).filter(Boolean))]}
        priceRange={{ min: 0, max: Math.max(...books.map(p => Number(p.price) || 0)) }}
      />
          ...
          >
            ...
            >
              <div className="w-full h-48 bg-gray-50 rounded mb-4 overflow-hidden flex items-center justify-center">
                <img
                  src={(product.images && product.images[0]) || product.imageUrl || BookImage}
                  alt={product.name}
