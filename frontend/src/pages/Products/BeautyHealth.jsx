...
function BeautyHealth() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/products?category=BeautyHealth");
        if (!res.ok) throw new Error("Failed to fetch beauty products");
        const data = await res.json();
        const productsArray = data.products || data || [];
        setBeautyProducts(productsArray);
        setFilteredProducts(productsArray);
      } catch (err) {
        console.error("❌ Failed to load beauty products:", err);
      } finally {
  ...
  return (
    <div className="min-h-screen text-black bg-gray-100 p-6">
      ...
      <ProductFilter
        ...
        brands={[...new Set(beautyProducts.map(p => p.brand).filter(Boolean))]}
        priceRange={{ min: 0, max: Math.max(...beautyProducts.map(p => Number(p.price) || 0)) }}
      />
        ...
        {filteredProducts.map((product) => (
          ...
          >
            ...
            <Link
              ...
              state={{ product, category: "BeautyHealth" }}
