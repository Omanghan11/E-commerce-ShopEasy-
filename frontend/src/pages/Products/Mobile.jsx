  ...
  useEffect(() => {
    ...
    const fetchMobiles = async () => {
      try {
        ...
        setMobileProducts(productsArray);
    ...
    <div className="min-h-screen text-black bg-gray-100 p-6">
      ...
      >
        ...
        <div className="relative z-10 px-4">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-3 sm:mb-4 drop-shadow-lg">Mobile Products</h1>
          <p className="text-base sm:text-lg mb-4 sm:mb-6 drop-shadow-md">Explore our range of mobile products.</p>
          <Link
            to="/"
      ...
      <ProductFilter
        ...
        brands={[...new Set(mobileProducts.map(p => p.brand).filter(Boolean))]}
        priceRange={{ min: 0, max: Math.max(...mobileProducts.map(p => Number(p.price) || 0)) }}
      />
          ...
          >
            ...
            >
              <div className="w-full h-48 bg-gray-50 rounded mb-4 overflow-hidden flex items-center justify-center">
                <img
                  src={(product.images && product.images[0]) || product.imageUrl || MobileImage}
                  alt={product.name}
