...
import SportsBanner from "../../assets/Images/Sports.jpg";
    ...
    const fetchSports = async () => {
      try {
        const res = await fetch(
          "https://shopeasy-backend-sagk.onrender.com/api/products?category=SportsOutdoors"
        );
        if (!res.ok) throw new Error("Failed to fetch products");
        const data = await res.json();
        const productsArray = data.products || data || [];
        setSportProducts(productsArray);
    ...
    <div className="min-h-screen text-black bg-gray-100 p-6">
      ...
      >
        ...
        <div className="relative z-10 px-4">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-3 sm:mb-4 drop-shadow-lg">Sports & Outdoors</h1>
          <p className="text-base sm:text-lg mb-4 sm:mb-6 drop-shadow-md">
            Explore our range of sports and outdoor products.
          </p>
          <Link
            to="/"
      ...
      <ProductFilter
        ...
        brands={[...new Set(sportProducts.map(p => p.brand).filter(Boolean))]}
        priceRange={{ min: 0, max: Math.max(...sportProducts.map(p => Number(p.price) || 0)) }}
      />
        ...
        {filteredProducts.map((product) => (
          ...
          >
            ...
            <button
              ...
              className={`absolute top-3 right-3 flex items-center justify-center w-10 h-10 rounded-full transition ${
                isInWishlist(product._id) && isLoggedIn
                  ...
                  : 'border-2 border-gray-400 bg-white hover:bg-gray-50 text-gray-500 cursor-pointer'
              }`}
            ...
            <Link
              ...
              state={{ product, category: "SportsOutdoors" }}
            ...
            >
              <div className="w-full h-48 bg-gray-50 rounded mb-4 overflow-hidden flex items-center justify-center">
                <img
                  src={(product.images && product.images[0]) || product.imageUrl || SportsBanner}
                  alt={product.name}
